...
func main() {
	...
	case "refl":
		...
		refl.Start()
	case "rcv":
		rcv, err := receiver.NewRclipRcv(filepath.Join(conf_dir, "ca_cert"), filepath.Join(conf_dir, "rcv_cert"), filepath.Join(conf_dir, "rcv_key"), *refl_ip, int(*rcv_port))
		if err != nil {
			common.ErrLog.Println(err)
			return
		}
	...
	case "sender":
		snd, err := sender.NewRclipSender(filepath.Join(conf_dir, "ca_cert"), filepath.Join(conf_dir, "sender_cert"), filepath.Join(conf_dir, "sender_key"), *refl_ip, int(*send_port))
		if err != nil {
			common.ErrLog.Println(err)
			return
		}
